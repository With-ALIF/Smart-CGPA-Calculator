...
function WhatIfCalculator({ baseCgpa, rows }) {
  const [changes, setChanges] = useState([{ subjectIndex: 0, targetGrade: 'A' }]);
  ...
  const result = useMemo(() => {
    const totalCredits = rows.reduce((sum, row) => sum + Number(row.credit), 0);
    if (!totalCredits) return Number(baseCgpa.toFixed(2));

    const basePoints = rows.reduce((sum, row) => sum + Number(row.credit) * gradeScale[row.grade].gpa, 0);
    const adjustedPoints = rows.reduce((sum, row, index) => {
      const change = changes.find((item) => item.subjectIndex === index);
      const grade = change ? change.targetGrade : row.grade;
      return sum + Number(row.credit) * gradeScale[grade].gpa;
    }, 0);
    ...
    <section className="rounded-[28px] border border-white/70 bg-white/70 p-5 shadow-xl shadow-slate-200/60 backdrop-blur-xl dark:border-slate-800/80 dark:bg-slate-900/70 dark:shadow-black/30">
      ...
      <div className="space-y-3">
        {changes.map((change, index) => (
          <div key={index} className="grid gap-3 rounded-2xl border border-slate-200 bg-slate-50/80 p-3 dark:border-slate-700 dark:bg-slate-800/80 md:grid-cols-[1fr_1fr_auto]">
            <label>
              <span className="mb-2 block text-sm font-medium">Subject</span>
              <select value={change.subjectIndex} onChange={(e) => updateChange(index, 'subjectIndex', Number(e.target.value))} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 outline-none dark:border-slate-700 dark:bg-slate-900">
            ...
            <label>
              <span className="mb-2 block text-sm font-medium">Change to</span>
              <select value={change.targetGrade} onChange={(e) => updateChange(index, 'targetGrade', e.target.value)} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 outline-none dark:border-slate-700 dark:bg-slate-900">
                {Object.entries(gradeScale).map(([grade, { label }]) => <option key={grade} value={grade}>{label}</option>)}
              </select>
            </label>
            <button onClick={() => removeChange(index)} className="flex items-center justify-center rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-sm font-medium text-rose-600 transition hover:bg-rose-100 dark:border-rose-800 dark:bg-rose-900/20 dark:text-rose-300">
              <Trash2 size={16} />
            </button>
          </div>
        ))}
