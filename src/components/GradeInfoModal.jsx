...
function GradeInfoModal({ onClose }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/70 px-4 py-6 backdrop-blur-sm">
      <div className="flex min-h-full items-center justify-center">
        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }} onClick={(e) => e.stopPropagation()} className="relative w-full max-w-2xl rounded-[30px] border border-white/70 bg-white/90 shadow-2xl shadow-slate-900/20 backdrop-blur-xl dark:border-slate-800/80 dark:bg-slate-900/90">
          <div className="sticky top-0 z-10 flex items-center justify-between rounded-t-[30px] border-b border-slate-200/60 bg-white/95 px-6 pt-6 pb-4 backdrop-blur-xl dark:border-slate-700/60 dark:bg-slate-900/95">
            <div>
          ...
          <div className="max-h-[60vh] overflow-y-auto px-6 py-4">
            ...
            <div className="mt-6 rounded-2xl border border-violet-200 bg-gradient-to-br from-violet-50 to-cyan-50 p-4 dark:border-violet-800 dark:from-slate-800 dark:to-slate-700">
              ...
              <p className="mt-2 text-sm font-bold text-slate-600 dark:text-slate-300">CGPA = Σ(Grade Point × Credit) ÷ Σ(Credit)</p>
